...
function StatusPill({ status }) {
    ...
    return <span className={`inline-flex text-[10px] px-2 py-1 font-extrabold uppercase tracking-widest border-2 border-black ${s.c}`}>{s.t}</span>;
}
    ...
    return (
        <div className="min-h-[100dvh] bg-[#F9F8F6] flex" data-testid="dashboard-page">
            ...
            <aside className="w-64 border-r-2 border-black bg-white p-6 hidden md:flex flex-col justify-between sticky top-0 h-screen">
                    ...
                    <nav className="mt-10 space-y-1">
                        {[
                            ...
                            <button
                                key={k}
                                onClick={() => setTab(k)}
                                className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-extrabold uppercase tracking-wider transition-colors ${tab === k ? "bg-[#FC8019] text-white border-2 border-black" : "border-2 border-transparent hover:bg-[#FFF3E7]"}`}
                ...
                <div>
                    <button onClick={() => nav("/kds")} className="ghost-btn w-full px-3 py-2 text-xs mb-3" data-testid="open-kds-btn">Open KDS →</button>
                    <div className="p-3 border-2 border-black bg-[#F9F8F6]">
                ...
                <div className="border-b-2 border-black bg-white sticky top-0 z-30">
                    <div className="px-6 py-4 flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            {tenant && (
                                <button onClick={() => copy(menuUrl)} className="ghost-btn px-3 py-2 text-xs inline-flex items-center gap-2" data-testid="copy-menu-url-btn">
                                    <Copy size={14} /> {menuUrl.replace(/^https?:\/\//, "")}
                                </button>
                            )}
                            <a href={menuUrl} target="_blank" rel="noreferrer" className="brand-btn px-3 py-2 text-xs inline-flex items-center gap-2" data-testid="open-menu-btn">
    ...
    return (
        <div className="space-y-8 fade-in" data-testid="tab-overview">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 hard-border bg-white p-6">
                ...
                <div className="hard-border bg-[#FC8019] text-black p-6">
        ...
        <div className="fade-in" data-testid="tab-menu">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 stagger">
                {dishes.map((d) => {
                    ...
                    return (
                        <div key={d.id} className="hard-border bg-white overflow-hidden" data-testid={`dish-card-${d.id}`}>
                            ...
                            <div className="p-4">
                                ...
                                <div className="mt-4 flex flex-wrap gap-2">
                                    <button onClick={() => setEditing(d)} className="ghost-btn px-3 py-2 text-xs inline-flex items-center gap-2" data-testid={`upload-video-btn-${d.id}`}>
                                        <VideoCamera size={14} weight="bold" /> {d.model_status === "ready" ? "Replace 3D" : "Upload video"}
                                    </button>
                                    <button onClick={() => toggleAvail(d)} className="ghost-btn px-3 py-2 text-xs" data-testid={`toggle-avail-${d.id}`}>
                                        {d.is_available ? "Hide" : "Show"}
                                    </button>
                                    <button onClick={() => delDish(d)} className="ghost-btn px-3 py-2 text-xs inline-flex items-center gap-2 hover:bg-red-100" data-testid={`delete-dish-${d.id}`}>
                ...
                {dishes.length === 0 && (
                    <div className="col-span-full hard-border p-10 text-center text-gray-500">No dishes yet. Click <b>New dish</b> to add one.</div>
                )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 backdrop-fade" data-testid="video-upload-modal">
            <div className="bg-white hard-border hard-shadow-lg w-full max-w-lg p-6 modal-enter">
                ...
                <div className="mt-6 flex justify-end gap-3">
                    <button onClick={onClose} className="ghost-btn px-4 py-3 text-sm" data-testid="video-cancel-btn">Cancel</button>
                    <button disabled={!file || uploading} onClick={submit} className="brand-btn px-4 py-3 text-sm" data-testid="video-submit-btn">
...
function TablesTab({ tables, tenant, onChanged }) {
    ...
    return (
        <div className="fade-in" data-testid="tab-tables">
            ...
            <div className="hard-border bg-white p-4 flex flex-wrap items-end gap-4 mb-8">
                <label className="block">
                    <span className="text-[10px] uppercase tracking-widest font-extrabold">Code</span>
                    <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="T09" className="mt-1 px-3 py-2 hard-border bg-white uppercase" data-testid="new-table-code" />
                </label>
                <label className="block">
                    <span className="text-[10px] uppercase tracking-widest font-extrabold">Seats</span>
                    <input type="number" min="1" value={seats} onChange={(e) => setSeats(e.target.value)} className="mt-1 w-24 px-3 py-2 hard-border bg-white" data-testid="new-table-seats" />
                </label>
                <button onClick={add} className="brand-btn px-4 py-3 text-sm inline-flex items-center gap-2" data-testid="add-table-btn"><Plus size={16} /> Add table</button>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 stagger">
                {tables.map((t) => {
                    ...
                    return (
                        <div key={t.id} className="hard-border bg-white p-4" data-testid={`table-card-${t.code}`}>
                            <div className="flex items-center justify-between">
                            ...
                            <div className="mt-3 aspect-square bg-white hard-border grid place-items-center">
                                <img src={qrSrc} alt={`QR for ${t.code}`} className="w-full h-full object-contain" />
                            </div>
                            <div className="mt-3 flex gap-2">
                                <button onClick={() => copy(url)} className="ghost-btn px-2 py-1.5 text-[10px] flex-1" data-testid={`copy-url-${t.code}`}>Copy URL</button>
                                <a href={url} target="_blank" rel="noreferrer" className="pill-orange px-3 py-1.5 text-[10px] flex-1 text-center" data-testid={`open-url-${t.code}`}>Open →</a>
                            </div>
                        </div>
                    );
                })}
    ...
    return (
        <div className="fade-in" data-testid="tab-orders">
            <div className="mb-6 flex items-end justify-between flex-wrap gap-3">
                ...
                <button onClick={onChanged} className="ghost-btn px-3 py-2 text-xs inline-flex items-center gap-2" data-testid="orders-refresh-btn"><ArrowClockwise size={14} /> Refresh</button>
            </div>
